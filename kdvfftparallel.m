...
for n = 1:nmax-40000
    ...
    spmd(q)
        c = ceil(labindex / 2); %index of current worker
        linear = (mod(labindex,2) == 0);
        resul = integrator(delta_t, U, k, c, linear, c);
    end

    for m = 1:s
        index = 2 * m;
        phi_positive = resul{index-1};
        phi_negative = resul{index};
